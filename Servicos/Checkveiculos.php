...
if(isset($msg)){
	$tpl = file_get_contents('./tpls/checkVeiculo/msg.html');	$tpl = str_replace('{{msg}}', $msg, $tpl);
...
$url = 'https://checkbusca.com/sixapi/vec.php?token=beb99570f3a3f81cf289a586e5abd89b&dados=';
...
if(isset($_POST['dados'])) {
    ...
    if(strlen($placa) > 3 && strlen($placa) < 50) {
    	...
    	if(stristr($dados, 'nada_encontrado')) {
	    	echo json_encode(array('msg' => 'nadaencontrado'));
    	}elseif(stristr($dados, 'nada encontrado')){
	    	echo json_encode(array('msg' => 'nadaencontrado'));
    	}elseif(stristr($dados, '{')) {
...
$tpl = file_get_contents('./tpls/checkVeiculo/index.html');
$tpl = str_replace('LIMITE:&ensp;<strong>&ensp;0</strong>&ensp;-&ensp;USADO:&ensp;<strong>&ensp;0</strong>', "LIMITE:&ensp;<strong>&ensp;{$a['limite']}</strong>&ensp;-&ensp;USADO:&ensp;<strong>&ensp;{$a['usado']}</strong>", $tpl);
$tpl = str_replace(array("\n", "\r", "\t", "  ", "	"), '', $tpl);
echo $tpl;
die;
